...
//     SYS_LockReg(); 
// }
//过一段时间要把sys.c和clk.c拼合成一个文件sys.c
//clksource为时钟的来源
//0为内部，即自带的22.1184MHz
//1为外部
//clk是所需要设置的频率,如果为外部时钟：12表示12MHz,32表示32KHz,
//若为内部时钟时：22表示内部的22.1184MHz,10表示10K
u8 MINI51_Clock_Init(u8 clksource,u8 clk)
{
    u32 i=0;
    SYS_UnlockReg();
    if(clksource==1)//外部时钟配置
    {
        CLK->PWRCON&=0XFFFFFFFC;
        if(clk==12)//12MHz
            CLK->PWRCON|=1<<0;
        else if(clk==32)
            CLK->PWRCON|=2<<0;
        CLK->PWRCON&=~(1<<2);//禁止22.1184
        CLK->PWRCON&=~(1<<3);//禁止10K
        ////等待时钟源是否稳定
        while((CLK->CLKSTATUS & 0x01)!=0x01)
        {
            i++;
            if(i>=1000)break;
        }       
        //设置HCLK及滴答时钟
        CLK->CLKSEL0&=0XFFFFFFC0;//HCLK来自外部
//         CLK->CLKSEL0|=3<<3;//Systick来自HCLK/2
//         CLK->CLKDIV&=0XFFFFFFF0;//分频因子为0，即除以1
    }
    else if(clksource==0)
    {
        //PWRCON保持默认即可
        ...
        //设置HCLK及滴答时钟
        CLK->CLKSEL0&=0XFFFFFFC0;
        if(clk==22)
            CLK->CLKSEL0|=7<<0;
        else if(clk==10)
            CLK->CLKSEL0|=3<<0;
//         CLK->CLKSEL0|=3<<3;//systick时钟来源于HCLK/2       
//         CLK->CLKDIV&=0XFFFFFFF0;//分频因子为0，即除以1
    }
    CLK->CLKSEL0|=3<<3;//systick时钟来源于HCLK/2       
    CLK->CLKDIV&=0XFFFFFFF0;//分频因子为0，即除以1
