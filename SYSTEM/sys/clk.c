...
//     SYS->P5_MFP|=3<<0;                                          //P5.0和P5.1配置为XTAL
//     SYS->P5_MFP&=~(3<<8);    
    CLK->PWRCON |=CLK_PWRCON_IRC22M_EN_Msk;//打开外部12MHZ时钟和内部22.1184MHz时钟
    CLK->PWRCON |=CLK_PWRCON_IRC10K_EN_Msk;//打开内部10KHz时钟用于开门狗的使用
    CLK_WaitClockReady(CLK_CLKSTATUS_IRC22M_STB_Msk);//等待时钟就绪   
    CLK_SetHCLK(CLK_CLKSEL0_HCLK_S_IRC22M,CLK_CLKDIV_HCLK(1));    //设置22   22/2=11   
    
    
    CLK_SetSysTickClockSrc(CLK_CLKSEL0_STCLK_S_HCLK_DIV2);      //设置SYSTICK时钟为系统时钟(HCLK)的一半      
